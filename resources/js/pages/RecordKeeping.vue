...
import type { ApplicantRecord, EmployeeRecordFileSummary } from '@/types';
...
const selectedApplicant = computed<ApplicantRecord | null>(() =>
    previewKind.value === 'applicant'
        ? (applicantRecords.find(
              (record) => record.id === previewRecordId.value,
          ) ?? null)
        : null,
);
...
const selectedEmployeeFile = computed<EmployeeRecordFileSummary | null>(() =>
    previewKind.value === 'employee'
        ? (employeeRecordFiles.find(
              (record) => record.employeeId === previewRecordId.value,
          ) ?? null)
        : null,
);
...
function filingStatusClass(
    status: EmployeeRecordFileSummary['filingStatus'],
): string {
    if (status === 'Complete') {
        return 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300';
    }
    ...
    <div class="flex flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        >
            <div class="flex flex-col gap-6">
                ...
                <div
                    class="rounded-3xl border border-emerald-200 bg-emerald-50/70 p-4 dark:border-emerald-900 dark:bg-emerald-950/20"
                >
                    <div class="flex items-start gap-3">
                        <div
                            class="rounded-2xl border border-emerald-200 bg-white/80 p-2.5 text-emerald-700 dark:border-emerald-900 dark:bg-background/60 dark:text-emerald-300"
                        >
                            <ShieldCheck class="size-5" />
                        </div>
                        <div class="space-y-1">
                            <p class="font-medium text-foreground">
                                Recruitment records start as applicants, not
                                employees
                            </p>
                            <p class="text-sm text-muted-foreground">
                                In this mockup, documents can be filed under an
                                applicant folder first. A person only moves into
                                the employee 201 file after appointment or
                                hiring.
                            </p>
                        </div>
                    </div>
        ...
        <div class="grid flex-1 gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
            <aside class="xl:sticky xl:top-4 xl:self-start">
                <Card class="rounded-[1.75rem] shadow-xs">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Switch between record areas without depending on the
                            Employees module.
                        </CardDescription>
                    </CardHeader>
            ...
            <div class="space-y-6">
                    ...
                    <Card class="rounded-[1.75rem] shadow-xs">
                        ...
                        >
                            <div>
                                ...
                                <CardDescription>
                                    Recruitment folders can be created first,
                                    even before a person becomes an employee.
                                </CardDescription>
                            </div>
                            <Badge
                                variant="outline"
                                class="rounded-full border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300"
                            >
                ...
                <section
                    v-else-if="activeSection === 'employee-files'"
                    class="space-y-4"
                >
                    <Card class="rounded-[1.75rem] shadow-xs">
                        <CardHeader
                        ...
                        >
                            <div>
                                ...
                                <CardDescription>
                                    Official personnel records created only
                                    after hiring, appointment, or onboarding.
                                </CardDescription>
                            </div>
                            <Badge
                                variant="outline"
                                class="rounded-full border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-900 dark:bg-blue-950/40 dark:text-blue-300"
                            >
                                Post-hiring only
                            </Badge>
                        </CardHeader>
                        <CardContent class="p-0">
                            <div class="overflow-x-auto">
                                <table class="min-w-full text-sm">
                                    ...
                                    <tbody>
                                        <tr
                                            v-for="employeeFile in employeeRecordFiles.slice(
                                                0,
                                                12,
                                            )"
                                        ...
                                        >
                                            <td class="px-6 py-4">
                                                <div class="space-y-1">
                                                    ...
                                                    >
                                                        {{
                                                            employeeFile.employeeCode
                                                        }}
                                                        ·
                                                        {{
                                                            employeeFile.departmentName
                                                        }}
                                                    </p>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ employeeFile.position }}
                                            </td>
                                            <td class="px-6 py-4">
                                                <Badge
                                                    variant="outline"
                ...
                <section
                    v-else-if="activeSection === 'registry'"
                    class="space-y-4"
                >
                    <Card class="rounded-[1.75rem] shadow-xs">
                        <CardHeader class="border-b bg-muted/20 pb-5">
                            ...
                            <CardDescription>
                                Central filing view across applicant folders and
                                employee 201 jackets.
                            </CardDescription>
                        </CardHeader>
                        <CardContent class="p-0">
                            <div class="overflow-x-auto">
                ...
                <section v-else class="space-y-4">
                    <Card class="rounded-[1.75rem] shadow-xs">
                        <CardHeader class="border-b bg-muted/20 pb-5">
                            ...
                            <CardDescription>
                                Archived record folders and retained forms with
                                storage references.
                            </CardDescription>
                        </CardHeader>
                        <CardContent class="p-0">
        ...
        <Sheet :open="isPreviewOpen" @update:open="isPreviewOpen = $event">
            ...
            >
                <div
                    ...
                    v-if="selectedApplicant || selectedEmployeeFile"
                >
                    <SheetHeader class="border-b px-6 pt-6 pb-5 text-left">
                        <SheetTitle>
                            {{
                                selectedApplicant?.name ??
                                selectedEmployeeFile?.employeeName
                            }}
                        </SheetTitle>
                        <SheetDescription>
                            {{
                    ...
                    <div class="space-y-4 px-6 py-6">
                        <Card
                            ...
                            class="rounded-3xl shadow-none"
                        >
                            <CardHeader>
                                <CardTitle>Applicant record summary</CardTitle>
                                <CardDescription>
                                    Recruitment documents may exist here before
                                    any employee profile is created.
                                </CardDescription>
                            </CardHeader>
                            <CardContent class="space-y-4">
                                ...
                                <div
                                    class="rounded-2xl border bg-blue-50/70 p-4 text-sm text-blue-800 dark:border-blue-900 dark:bg-blue-950/20 dark:text-blue-300"
                                >
                                    Once this applicant is hired or appointed,
                                    the folder can be promoted into an employee
                                    201 file without requiring a login account
                                    during recruitment.
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card
                            v-if="selectedEmployeeFile"
                            class="rounded-3xl shadow-none"
                        >
                            <CardHeader>
                                <CardTitle>Employee file summary</CardTitle>
                                <CardDescription>
                                    Official employee record kept independently
                                    from timekeeping and recruitment views.
                                </CardDescription>
                            </CardHeader>
                            <CardContent class="space-y-4">
                                <div class="grid gap-4 sm:grid-cols-2">
