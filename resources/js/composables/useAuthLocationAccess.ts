...
import { locationAccessDenied } from '@/routes';
...
export function useAuthLocationAccess(
    ...
    watch(
        ...
        (value) => {
            ...
            locationFeedback.value = {
                kind: 'idle',
                message: `Access is restricted to users within ${value.radiusMeters} meters of the saved access point.`,
            };
        },
        {
            immediate: true,
    ...
    onMounted(() => {
        if (geofence.value) {
            void ensureAllowedLocation({
                locatingMessage: 'Checking your current location before opening this page...',
                ...
                (position) => {
                    ...
                    locationFeedback.value = {
                        ...
                        message:
                            ...
                            `Location verified at ${formatDistance(distanceMeters)} from the allowed area.`,
                    };
                    resolve(true);
                },
                () => {
    ...
    function redirectToDeniedPage(): void {
        isResolvingLocation.value = false;
        hasResolvedLocation.value = false;
        router.visit(locationAccessDenied(), {
            replace: true,
        });
    }
